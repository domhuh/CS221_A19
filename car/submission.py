...
class ExactInference(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        #raise Exception("Not implemented yet")  #(mean, std, value)
        for row in range(self.belief.numRows):
            for col in range(self.belief.numCols):
                ...
                distribution = util.pdf(distance,Const.SONAR_STD,observedDist) * self.belief.getProb(row,col)
                self.belief.setProb(row,col,distribution)
    ...
    def elapseTime(self):
        ...
        for oldTile, newTile in self.transProb:
            value=self.transProb[(oldTile,newTile)]*self.belief.getProb(oldTile[0],oldTile[1])
            if value>0: newBelief.addProb(newTile[0],newTile[1],value)
        newBelief.normalize()
...
class ParticleFilter(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for row,col in self.particles:
            distance = math.sqrt((util.colToX(col)-agentX)**2+(util.rowToY(row)-agentY)**2)
            distribution=util.pdf(distance,Const.SONAR_STD,observedDist)*self.particles[(row,col)]
            resamplingPool[(row,col)]= resamplingPool.get((row,col),0)+distribution
        self.particles={}
        for i in range(0,self.NUM_PARTICLES):
            choice=util.weightedRandomChoice(resamplingPool)
            self.particles[choice]=self.particles.get(choice,0)+1
        # END_YOUR_CODE
        ...
        for tile in self.particles:
            if tile in self.transProbDict:
                ...
                for i in range(value):
                    resamplingPool=self.transProbDict[tile]
                    choice=util.weightedRandomChoice(resamplingPool)
                    newParticles[choice]=newParticles.get(choice,0)+1
