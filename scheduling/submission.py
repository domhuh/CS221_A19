...
def create_chain_csp(n):
    ...
    for idx,var in enumerate(variables):
        ...
        if idx > 0: csp.add_binary_factor(var, variables[idx - 1], lambda a,b: a!=b)
    # END_YOUR_CODE
    return csp
...
def create_nqueens_csp(n = 8):
    ...
    #raise Exception("Not implemented yet")
    queens=[f'Queen {i}' for i in range(1,n+1)]
    for idx, var in enumerate(queens):
        csp.add_variable(var,[(idx,i) for i in range(0,n)])
    for var1 in queens:
        for var2 in queens:
            if var1 == var2: continue
            csp.add_binary_factor(var1,var2,lambda a,b:a[1]!=b[1])
            csp.add_binary_factor(var1,var2,lambda a,b:(a[0]-a[1])!=(b[0]-b[1]) and (a[0]+a[1])!=(b[0]+b[1]))
    # END_YOUR_CODE
    return csp
...
class BacktrackingSearch():
    ...
    def get_unassigned_variable(self, assignment):
        ...
        if not self.mcv:
            ...
            # BEGIN_YOUR_CODE (our solution is 7 lines of code, but don't worry if you deviate from this)
            #raise Exception("Not implemented yet")
            mcv, domainSize=None, float("inf")
            for var in self.csp.variables:
                if var not in assignment:
                    dSize=0
                    for val in self.domains[var]:
                        dw = self.get_delta_weight(assignment, var, val)
                        if dw > 0: dSize+=1
                    if domainSize>dSize:
                        domainSize=dSize
                        mcv=var
    ...
    def arc_consistency_check(self, var):
        ...
        while len(queue)!=0:
            v1 = queue.pop(0)
            for vn in self.csp.get_neighbor_vars(v1):
                inconsistent=[]
                for vnd in self.domains[vn]:
                    binaryFlag=False
                    for v2 in self.domains[v1]:
                        if self.csp.binaryFactors[vn][v1][vnd][v2]>0: binaryFlag=True
                    if not binaryFlag:
                        inconsistent.append(vnd)
                if len(inconsistent)==0: continue
                for value in inconsistent:
                    self.domains[vn].remove(value)
                for vnn in self.csp.get_neighbor_vars(vn):
                    queue.append(vn)
        # END_YOUR_CODE
...
def get_sum_variable(csp, name, variables, maxSum):
    ...
    for idx in range(len(variables)):
        Xi=('sum',name,idx)
        if idx==0:
            csp.add_variable(Xi,[(pastXi,out) for out in range(maxSum+1)])
            csp.add_binary_factor(Xi,variables[0],lambda aux,xi:aux[1]==xi)
        else:
            csp.add_variable(Xi,[(pastSum,possibleSums) for pastSum in range(maxSum+1) for possibleSums in range(maxSum+1)])
            csp.add_binary_factor(Xi,pastXi,lambda newAux,oldAux: newAux[0]==oldAux[1])
            csp.add_binary_factor(Xi,variables[idx],lambda aux,xi: aux[1]==(aux[0]+xi))
        pastXi=Xi
    csp.add_variable(newVar, list(range(maxSum+1)))
    csp.add_binary_factor(newVar,Xi,lambda x,y:x==y[1])
    return newVar
    # END_YOUR_CODE
            ...
            if len(req.quarters) != 0:
                for quarter in self.profile.quarters:
                    if quarter not in req.quarters:
                        csp.add_unary_factor((req, quarter), lambda cid: cid == None)
        # END_YOUR_CODE
        ...
        for quarter in self.profile.quarters:
            ...
            for request in self.profile.requests:
                for cid in request.cids:
                    newCourse =(cid,quarter)
                    csp.add_variable(newCourse,[0,*list(range(self.bulletin.courses[cid].minUnits,self.bulletin.courses[cid].maxUnits + 1))])
                    csp.add_binary_factor((request,quarter), newCourse, lambda request_cid,course_unit,cid=cid: course_unit if cid == request_cid else course_unit==0)
                    allCourses.append(newCourse)

            totalUnits=get_sum_variable(csp, quarter, allCourses, self.profile.maxUnits)
            csp.add_unary_factor(totalUnits, lambda n: n >=self.profile.minUnits and n<=self.profile.maxUnits)
        # END_YOUR_CODE
